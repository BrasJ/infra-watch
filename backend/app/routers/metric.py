...
def register_metric(metric: MetricCreate, db: Session = Depends(get_db)):
    return create_metric(db, metric)

@router.get("/snapshot/{snapshot_id}", response_model=List[MetricRead])
def list_metrics_by_snapshot(snapshot_id: int, db: Session = Depends(get_db)):
...
def remove_metric(metric_id: int, db: Session = Depends(get_db)):
